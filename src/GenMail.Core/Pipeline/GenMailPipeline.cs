...
public sealed class GenMailPipeline
{
    public async Task<ProcessingResult> RunAsync(string inputPath, GenerationOptions options, IProgress<ProgressSnapshot>? progress, CancellationToken cancellationToken)
    {
        ...
        void OpenWriters()
        {
            usernamesWriter = new StreamWriter(new FileStream(CurrentUsernamesPath(), FileMode.Create, FileAccess.Write, FileShare.Read, 4 * 1024 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan), new UTF8Encoding(false), 4 * 1024 * 1024);
            emailsWriter = new StreamWriter(new FileStream(CurrentEmailsPath(), FileMode.Create, FileAccess.Write, FileShare.Read, 4 * 1024 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan), new UTF8Encoding(false), 4 * 1024 * 1024);
            outputFilesCreated++;
            rowsInCurrentFile = 0;
        }
