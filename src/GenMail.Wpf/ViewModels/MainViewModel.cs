...
public sealed class MainViewModel : INotifyPropertyChanged
{
    ...
    public bool SplitOutputFiles { get; set; } = true;
    ...
    private void Estimate()
    {
        ...
        if (!SplitOutputFiles && estimate.EstimatedOutput > 100000)
        {
            StatusText += " | Warning: enable split output for large runs.";
        }
        if (!SplitOutputFiles && estimate.EstimatedOutput > 1000000)
        {
            StatusText += " | Strong warning: output is very large without splitting.";
        }
