...
- Large `emails.txt` can be difficult to open in editors; enable split output for large runs.
- Recommended split rows per file: 50,000 or 100,000.
- SQLite dedupe stores keys only by default to reduce disk usage.
- InMemory dedupe is recommended for moderate runs.
- SQLite dedupe is useful for very large dedupe histories but still uses disk.
- If output files cannot be opened, ensure the run has finished or close the app.
- `dedupe.db`, `dedupe.db-shm`, and `dedupe.db-wal` can be deleted when dedupe history is no longer needed.
