...
public class LargeOutputUsabilityTests
{
    [Fact]
    public async Task Pipeline_OutputFiles_CanBeOpenedForReadAfterRun()
    {
        string input = Path.Combine(Path.GetTempPath(), $"gm_lock_{Guid.NewGuid():N}.txt");
        await File.WriteAllLinesAsync(input, new[] { "john smith", "jane doe" });
        GenerationOptions options = new GenerationOptions("example.com", Path.Combine(Path.GetTempPath(), "gm_lock_out"), SelectedRuleIds: new[] { "firstlast" });
        ProcessingResult result = await new GenMailPipeline().RunAsync(input, options, null, CancellationToken.None);

        using FileStream e = new FileStream(Path.Combine(result.OutputDirectory, "emails.txt"), FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        using FileStream u = new FileStream(Path.Combine(result.OutputDirectory, "usernames.txt"), FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        Assert.True(e.Length > 0);
        Assert.True(u.Length > 0);
    }

    [Fact]
    public async Task SqliteDedupe_DefaultSchema_IsKeyOnly()
    {
        string path = Path.Combine(Path.GetTempPath(), $"gm_dedupe_{Guid.NewGuid():N}.db");
        await using (SqliteDedupeStore store = new SqliteDedupeStore(path))
        {
            await store.TryAddAsync(new DedupeEntry("s", "m", "k", "u", "e", "src", DateTimeOffset.UtcNow), CancellationToken.None);
        }

        using Microsoft.Data.Sqlite.SqliteConnection conn = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={path}");
        conn.Open();
        using Microsoft.Data.Sqlite.SqliteCommand cmd = conn.CreateCommand();
        cmd.CommandText = "PRAGMA table_info(generated_keys);";
        using Microsoft.Data.Sqlite.SqliteDataReader r = cmd.ExecuteReader();
        List<string> cols = new List<string>();
        while (r.Read()) cols.Add(r.GetString(1));
        Assert.Equal(new[] { "scope", "key_mode", "dedupe_key" }, cols);
    }
}
